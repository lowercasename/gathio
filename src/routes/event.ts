    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (event.maxAttendees !== null && event.maxAttendees !== undefined) {
                const currentCount = event.attendees?.reduce(
                    (acc, a) => {
            ...
            if (isHostAdding) {
                ...
                return res.redirect(`/${req.params.eventID}?p=${encodeURIComponent(removalPassword)}&m=rsvppending`);
            } else {
                // No approval needed - just redirect with ?p
                return res.redirect(`/${req.params.eventID}?p=${encodeURIComponent(removalPassword)}`);
            }
        } catch (e) {
            addToLog(
                "addEventAttendee",
...
router.delete(
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            // Redirect back to event page in edit mode
            return res.redirect(`/${req.params.eventID}?e=${event.editToken}&m=denied`);
        } catch (e) {
            addToLog(
                "denyEventAttendee",
                "error",
