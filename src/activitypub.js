                ...
                new Promise((resolve) => {
                    ...
                    signAndSend(
                        ...
                        (err) => {
                            resolve(
                                err
                                    ? { error: err, actor: f.actorId }
                                    : { ok: true, actor: f.actorId },
                            );
                        },
                    );
                }),
...
export function processInbox(req, res) {
    ...
    } else if (type === "Create" && object?.type === "Note" && object.to) {
...
async function _handleFollow(req, res) {
    ...
    try {
        ...
        await prisma.follower.upsert({
            where: {
                followId_actorId: { followId: req.body.id, actorId: actorUrl },
            },
            create: {
...
async function _handleUndoAcceptEvent(req, res) {
    ...
    try {
        ...
        addToLog(
            "oneClickUnattend",
            "success",
            `Undo RSVP ${actorUrl} from ${eventID}`,
        );
        res.sendStatus(200);
    } catch (err) {
...
async function _handleCreateNoteComment(req, res) {
    ...
    if (
        ...
        // Extract eventID
        const targets = [...to, ...cc].filter((u) =>
            u.includes(`https://${domain}/`),
        );
        if (targets.length !== 1) {
            return res.sendStatus(200);
        }
        ...
        try {
            ...
            const name =
                parsedActor.preferredUsername || parsedActor.name || actorUrl;
