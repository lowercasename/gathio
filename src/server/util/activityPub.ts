...
// On Gathio, ActivityPub Actors are the events themselves.
const createActivityPubActor = ({
    eventID,
    domain,
    pubkey,
    description,
    ...
    if (startUTC && timezone) {
        displayDate = DateTime.fromISO(startUTC).toLocaleString({
    ...
    if (endUTC && timezone) {
        displayDate = DateTime.fromISO(endUTC).toLocaleString({
            ...DateTime.DATETIME_FULL,
            weekday: "long",
        });
        actor.summary += `<p>Ending ${displayDate} ${timezone}.</p>`;
    }
    if (imageFilename) {
        actor.icon = {
            type: "Image",
            mediaType: "image/jpg",
            url: `https://${domain}/uploads/${imageFilename}`,
        };
    }
    return JSON.stringify(actor);
};

const createActivityPubEvent = (
    name,
    startUTC,
    endUTC,
    timezone,
