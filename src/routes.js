    ...
    Event.findOne({
        ...
        .then((event) => {
            if (event) {
                ...
                let eventStartISO = moment
                    .tz(event.start, "Etc/UTC")
                    .toISOString();
                ...
                let eventAttendees = event.attendees
                    ...
                    .filter((obj, pos, arr) => {
                        return (
                            ...
                            arr.map((mapObj) => mapObj.id).indexOf(obj.id) ===
                                pos
                        );
                    });

                let spotsRemaining, noMoreSpots;
                let numberOfAttendees = eventAttendees.reduce(
                    (acc, attendee) => {
        ...
        event
            ...
            .then(() => {
                ...
                if (creatorEmail) {
                    sendEmailFromTemplate(
                        creatorEmail,
                        `${importedEventData.summary} created`,
                        "createEvent",
                        {
                            eventID,
                            editToken,
    ...
    eventGroup
        ...
        .then(() => {
            ...
            if (req.body.creatorEmail) {
                sendEmailFromTemplate(
                    req.body.creatorEmail,
                    `${req.body.eventGroupName} created`,
                    "createEventGroup",
                    {
                        eventGroupID,
                        editToken,
    ...
    Event.update(
        ...
        .then((response) => {
            ...
            if (req.body.attendeeEmail) {
                sendEmailFromTemplate(
                    req.body.attendeeEmail,
                    `You're no longer RSVPed to ${event.name}`,
                    "unattendEvent",
                    {
                        eventID: req.params.eventID,
                    },
...
router.post("/subscribe/:eventGroupID", (req, res) => {
    ...
    EventGroup.findOne({
        ...
        .then((eventGroup) => {
            ...
            sendEmailFromTemplate(
                subscriber.email,
                `You've subscribed to ${eventGroup.name}`,
                "subscribed",
                {
                    eventGroupName: eventGroup.name,
                    eventGroupID: eventGroup.id,
