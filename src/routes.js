...
schedule.scheduleJob("59 23 * * *", async () => {
  ...
  console.log("Old event deletion running! Deleting all events before ", too_old);

  try {
    const oldEvents = await prisma.event.findMany({ where: { end: { lte: too_old } } });
    for (const event of oldEvents) {
      // delete image file
      if (event.image) {
        fs.unlink(path.join(process.cwd(), `/public/events/${event.image}`), err => {
          if (err) addToLog("deleteOldEvents", "error", `Image deletion failed for ${event.id}: ${err}`);
          else addToLog("deleteOldEvents", "success", `Image deleted for ${event.id}`);
        });
      }
      ...
      if (event.activityPubActor && event.activityPubEvent) {
        ...
        broadcastDeleteMessage(actorObj, await getFollowers(event.id), event.id, async () => {
          broadcastDeleteMessage(eventObj, await getFollowers(event.id), event.id, async () => {
...
async function getFollowers(eventId) {
  ...
  return followers.map(f => ({ followId: f.followId, actorId: f.actorId, actorJson: f.actorJson, name: f.name }));
}
...
router.post("/deleteimage/:eventID/:editToken", async (req, res) => {
  ...
  fs.unlink(path.join(process.cwd(), `/public/events/${imageName}`), async err => {
    if (err) {
      await addToLog("deleteEventImage", "error", `Deleting image for ${eventID} failed: ${err}`);
      return res.status(500).send(err);
    }
    await prisma.event.update({ where: { id: eventID }, data: { image: "" } });
    addToLog("deleteEventImage", "success", `Image deleted for ${eventID}`);
    res.sendStatus(200);
  });
});
...
router.post("/deleteevent/:eventID/:editToken", async (req, res) => {
  const { eventID, editToken } = req.params;
  const event = await prisma.event.findUnique({ where: { id: eventID } });
  if (!event || event.editToken !== editToken) return res.status(403).send("Invalid token");

  // broadcast delete
  if (event.activityPubActor) {
    const actorObj = JSON.parse(event.activityPubActor);
    await broadcastDeleteMessage(actorObj, await getFollowers(eventID), eventID, async () => {
...
async function performEventDeletion(event) {
  ...
  if (event.image) {
    fs.unlink(path.join(process.cwd(), `/public/events/${event.image}`), err => {
      if (err) addToLog("deleteEvent", "error", `Deleting image for ${eventID} failed: ${err}`);
    });
  }

  // notify attendees
  const attendees = await prisma.attendee.findMany({ where: { eventId: eventID, status: "attending", email: { not: null } } });
  if (attendees.length) {
...
router.post("/deleteeventgroup/:eventGroupID/:editToken", async (req, res) => {
  ...
  const group = await prisma.eventGroup.findUnique({ where: { id: eventGroupID } });
  if (!group || group.editToken !== editToken) return res.status(403).send("Invalid token");

  // unlink events
  await prisma.event.updateMany({ where: { eventGroupId: eventGroupID }, data: { eventGroupId: null } });
  // delete group record
  ...
  // delete group image
  if (group.image) {
    fs.unlink(path.join(process.cwd(), `/public/events/${group.image}`), err => {
      if (err) addToLog("deleteEventGroup", "error", `Deleting group image failed: ${err}`);
    });
  }
...
router.post("/attendee/provision", async (req, res) => {
  ...
  if (event.maxAttendees != null) {
    const attending = await prisma.attendee.findMany({ where: { eventId: eventID, status: "attending" } });
...
router.post("/attendevent/:eventID", async (req, res) => {
  ...
  const provision = event.attendees.find(a => a.removalPassword === removalPassword && a.status === 'provisioned');
  ...
  if (event.maxAttendees != null) {
    ...
    const used = attending.reduce((acc,a) => acc + (a.number||1), 0);
    if (attendeeNumber > event.maxAttendees - used) return res.sendStatus(403);
  }

  // update attendee
  await prisma.attendee.update({ where: { id: provision.id }, data: {
    status: "attending",
    name: attendeeName,
    email: attendeeEmail,
  ...
  if (attendeeEmail) {
    req.emailService.sendEmailFromTemplate({
      ...
      templateData: { eventID, removalPassword, removalPasswordHash: hashString(removalPassword) },
    });
  }
...
router.get("/oneclickunattendevent/:eventID/:attendeeID", async (req, res) => {
  ...
  if (ua.includes('mastodon')||ua.includes('pleroma')) return res.sendStatus(200);

  const { eventID, attendeeID } = req.params;
  await prisma.attendee.deleteMany({ where: { id: attendeeID, eventId: eventID } });
  addToLog("oneClickUnattend", "success", `${attendeeID} un-attended ${eventID}`);
  res.redirect(`/${eventID}`);
});
...
router.post("/removeattendee/:eventID/:attendeeID", async (req, res) => {
  const { eventID, attendeeID } = req.params;
  await prisma.attendee.deleteMany({ where: { id: attendeeID, eventId: eventID } });
  addToLog("removeEventAttendee", "success", `${attendeeID} removed from ${eventID}`);
  res.redirect(`/${eventID}`);
});
...
router.post("/subscribe/:eventGroupID", async (req, res) => {
  ...
  await prisma.subscriber.create({ data: { email, eventGroupId: eventGroupID } });
  req.emailService.sendEmailFromTemplate({
    to: email,
    subject: i18next.t("routes.subscribedsubject"),
    templateName: "subscribed",
    templateData: { eventGroupName: (await prisma.eventGroup.findUnique({ where:{ id: eventGroupID } })).name, eventGroupID, emailAddress: encodeURIComponent(email) }
  });
...
router.get("/unsubscribe/:eventGroupID", async (req, res) => {
  ...
  if (!email) return res.sendStatus(400);
  await prisma.subscriber.deleteMany({ where: { email: String(email), eventGroupId: eventGroupID } });
  res.redirect("/");
});
...
router.post("/post/reply/:eventID/:commentID", async (req, res) => {
  ...
  addToLog("addEventReply", "success", `Reply added to ${commentID} in ${eventID}`);
...
// Delete comment
router.post("/deletecomment/:eventID/:commentID/:editToken", async (req, res) => {
  const { eventID, commentID, editToken } = req.params;
  const event = await prisma.event.findUnique({ where: { id: eventID } });
  if (!event || event.editToken !== editToken) return res.sendStatus(403);
  await prisma.comment.deleteMany({ where: { id: commentID, eventId: eventID } });
  addToLog("deleteComment", "success", `Comment ${commentID} deleted from ${eventID}`);
